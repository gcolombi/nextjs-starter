...
const getFormSchema = () => {
    ...
    return object({
        ...
        coverletter: mixed().test('fileType', 'Unauthorized format, only doc, docx and pdf are valid', (files) => !files || new RegExp(/[^\s]+(.*?).(docx?|pdf)$/i).test(files[0]?.name || files.originalFilename))
        .test('fileSize', 'Max file size 4MB exceeded', (files) => !files || (files[0]?.size || files.size) <= 4 * 1024 * 1024 ),
